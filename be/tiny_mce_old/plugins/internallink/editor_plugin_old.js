...
function TinyMCE_template_initInstance(inst) {
	...
	//alert("Initialization parameter:" + tinyMCE.getParam("internallink_someparam", false));
}

/**
 * Gets executed when a editor needs to generate a button.
 */
function TinyMCE_internallink_getControlHTML(control_name) {
	switch (control_name) {
		case "internallink":
			return '<img id="{$editor_id}_internallink" src="{$pluginurl}/images/internallink.gif" title="Iternal Link" width="20" height="20" class="mceButtonNormal" onmouseover="tinyMCE.switchClass(this,\'mceButtonOver\');" onmouseout="tinyMCE.restoreClass(this);" onmousedown="tinyMCE.restoreAndSwitchClass(this,\'mceButtonDown\');" onClick="tinyMCE.execInstanceCommand(\'{$editor_id}\',\'mceinternallink\', true);" />';
	}
 ...
 */
function TinyMCE_internallink_execCommand(editor_id, element, command, user_interface, value) {
	// Handle commands
	...
	switch (command) {
		...
		case "mceinternallink":
			...
			if (user_interface) {
				...
				alert("execCommand: mceinternallink gets called from popup.");
			}
 ...
 */
function TinyMCE_internallink_handleNodeChange(editor_id, node, undo_index, undo_levels, visual_aid, any_selection) {
	// Deselect internallink button
	//tinyMCE.switchClassSticky(editor_id + '_internallink', 'mceButtonNormal');
 ...
 */
function TinyMCE_internallink_cleanup(type, content) {
	switch (type) {
