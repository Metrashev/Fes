...
$db=getdb();
if(!$in_edit_id) {
	FE_Utils::_die("Invalid ID");
}

$row=$db->getRow("select * from boxes where id=?",array($in_edit_id));
if(empty($row)) {
	$db->Execute("insert into boxes (id) values(?)",array($in_edit_id));
	header("Location: ?id={$in_edit_id}");
	exit;
}
