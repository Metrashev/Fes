...
<a href="{$gallery_first['img']['l']}" target="_blank" id="LargeImageA"><img id="LargeImage" src="{$gallery_first['img']['m']}" alt="{$gallery_first['text']}" class="img_large"></a>
...
foreach ($data['gallery'] as $k=>$v) {
	$str.=<<<EOD
<td><a href="#img{$k}" onClick="ShowImageN($k); return false;"><img src="{$v['img']['s']}" alt="{$v['text']}" /></a></td>
EOD;
	$alt = str_replace("'", "\'", html_entity_decode($v['text'], ENT_QUOTES));
	$JSArray .= <<<EOD
a[$k] = new Array('{$alt}', '{$v['img']['m']}', '{$v['img']['l']}');
