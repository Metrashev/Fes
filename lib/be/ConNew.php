...
class FormatUtils {
	...
	static function decodeFormat($format,$value,$getArray=false,$emptyReturnVal='') {
		...
		//$months_full=array(1=>"РЇРЅСѓР°СЂРё",2=>"Р¤РµРІСЂСѓР°СЂРё",3=>"РњР°СЂС‚",4=>"пїЅ?РїСЂРёР»",5=>"РњР°Р№",6=>"Р®РЅРё",7=>"Р®Р»Рё",8=>"пїЅ?РІРіСѓпїЅ?С‚",9=>"РЎРµРїС‚РµРјРІСЂРё",10=>"РћРєС‚РѕРјРІСЂРё",11=>"пїЅ?РѕРµРјРІСЂРё",12=>"Р”РµРєРµРјРІСЂРё");
	...
	*/
	...
	function BuildQuery($params=null) {
		...
		if(!empty($this->CustomSelect)) {
			...
			$SQL=trim("select {$this->SelectFields} from {$this->Table}{$where}{$group}{$having}{$order} {$this->Limit}");
		}
		
		$this->load($SQL,$params);
	}
	
	function load($SQL,$params=null) {
		$db=getdb();
		/* @var $db CDB*/
		if(empty($params))
			$params=null;
			...
			$str="<table>
				<tr>
					...
					<td valign=\"middle\" style=\"width:20px;\">
						<input style=\"width:20px\" type=\"button\" onclick=\"_add_list('{$this->control_id}');\" value=\"&lt;&lt;\" />
						<br />
						<input style=\"width:20px\" type=\"button\" onclick=\"_remove_list('{$this->control_id}');\" value=\"&gt;&gt;\" />
					</td>";
				...
				$str.="<td valign=\"top\">
						<select".$this->getAttributesString().">".CLib::draw_listbox_options($this->values,null)."</select>
					</td>
				</tr>
			</table>
			";
...
<input type="button" value="" class="button_calendar" id="ib_{$name}"/>
<script>
Calendar.setup({inputField     :    '$name',          // id of the input field
          ifFormat       :    '{$this->control_array['FormatString']}',                          // format of the input field
...
class ControlValues {
	...
	function collectData($array,$postData,$useWrite=true,$skipFormatting=false,$validate=true,$repeat_index=null) {
...
function validateType($type,$value,$signed,$size=0,$acceptZeroDate=false) {
	...
	function getWriteData($array,$postData,$repeat_index=null) {
		$result=ControlValues::collectData($array,$postData,true,false,true,$repeat_index);
			...
			if($v['tagName']=='ManagedImage') {
				...
				if(!is_uploaded_file($tmp_name)) {
					if($v['parameters']['overwrite']) {
						...
						foreach ($v['parameters']['sizes'] as $pk=>$pv) {
							//$err=ControlValues::resizeImage($h,$v['parameters']['dir'].$v['parameters']['table'].'/'.$id.'_'.$v['parameters']['field'].'_'.$pk.$ext,$pv,$v['parameters']['resize']);
							//if(!empty($err)) {
							//	@unlink($v['parameters']['dir'].$v['parameters']['table'].'/'.$id.'_'.$v['parameters']['field'].'_'.$pk.$ext);
							//}
							//$tmp=$names_tmp;
							//$tmp[]=$last_name."_file_".$pk;
							//$t='$files'.Control_Utils::createArrayName($tmp,true);
							//eval("\$tf=$t;");
					...
					if(!CPictures::isAcceptableImage($ext)) {
						...
						if(empty($err)) {
							...
							if($update_db) {
								$db->execute("update {$v['parameters']['table']} set {$v['parameters']['field']}=? where id=?",array($ext,$id));
...
class DBValidator {
	...
	function isValidFloat($val,$signed) {
		if (is_string($val)) {
      		if (!ereg('^-?([0-9]+(\.[0-9]+)?|\.[0-9]+)([eE][-+]?[0-9]+)?$', $val)) 
...
//EOD;
	...
	if(isset($array['buttons'])) {
		...
		switch ($type) {
			case 'search': {
				$buttons=<<<EOD
				<input type="submit" name="search" value="Търси" />&nbsp;&nbsp;&nbsp;<input type="submit" name='btClear' value="Изчисти" />
EOD;
				break;
			}
			case 'edit': {
