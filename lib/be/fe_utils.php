...
class BE_Utils {
	...
	function loadTinyMceStyles($tag='styles') {
		$dir=dirname(__FILE__).'/../www/be/config/';
		$dir=realpath($dir);
		$result=array();
	...
	public static function loadTinyMce($body,$template) {
		$dir=dirname(__FILE__).'/../../be/config/tiny_mce/'.$template;
		if(file_exists($dir)) {
			$str=file_get_contents($dir);
			return str_replace("_#BODY#_",$body,$str);
		}
		return BE_Utils::getTinyMce($body,'/lib_be.css');
	}
...
<body>";
	...
	static function getDBImg($ref_id,$ref_table,$alt='',$style='') {
		...
		return empty($id)?'':"<img {$style} src='/be/showimg.php?id={$id}' alt='{$alt}' />";
	}
...
class CPictures {
	...
	static function createTumbnail($type,$src,$dst,$width=120,$height=120,$quality=100) {
		$info=-1;
		$attr=-1;
		list($width_orig, $height_orig,$info,$attr) = getimagesize($src);
		if($info==-1)
