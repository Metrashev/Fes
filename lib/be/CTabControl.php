...
class CTab {
	...
	function __construct($control_id,$add_full_screen=true) {
		...
		foreach ($this->tabs as $k=>$v) {
			...
			if($k==$this->active_tab&&$this->use_ajax&&isset($v['ajax'])) {
				...
				if(!isset($v['body'])&&$this->use_ajax) {
					$div[]="<div id='{$this->control_id}_div_{$k}' class='tabDiv' style='clear:both;display:{$display};'><div style='color:#993300;font-weight:bold;text-align:center;padding-top:15px;'><img align=\"absmiddle\" src=\"/be/i/loader.gif\" />Loading ...</div></div>";
				}
				else {
